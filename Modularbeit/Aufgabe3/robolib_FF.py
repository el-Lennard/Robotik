...
dhRoArm = np.array([[0.0,       0.0,      0.11574 ],
                    [-1.570796, 0.04105,  0.0     ],
                    [0.0,       0.16886,  0.0     ],
                    [0.0,       0.12792,  0.0     ],
                    [0.0,       0.10854,  0.01399 ],
                    [1.570796,  0.0,      0.01096 ]])

qRoArm = np.array([np.pi, -np.pi/2, 0.0, 0.0, 0.0, 0.0])
...
def T_2_rotvec(T):
    ...
    g = 1/(2*np.sin(Theta))
    rx = Theta * g * (R[2,1]-R[1,2])
    ry = Theta * g * (R[0,2]-R[2,0])
    rz = Theta * g * (R[1,0]-R[0,1])
...
def inv_RoArm(dh_para, pose, qGripper):
    ...
    # Koordinaten Transofrmation
    # Von TCP zurück auf Gelenk 4
   
    x = xp - a[4]*np.cos(a[4]) 
    z = zp + a[4]*np.sin(a[4])
    # Von Basis nach Gelenk2
    ...
    q11 = q12 = np.arctan2(xp,yp)-np.pi/2
    
    if((x**2 + z**2) <= (a[2]+a[3])):
        # Erste Lösung:
            
        
        test = np.array([a[2],a[3]])
        ...
        if(q31 < 0):  
            ...
            #psi = np.arccos((x**2 + z**2 - a[2]**2 - a[3]**2)/(2*a[2]*a[3]))
            beta = -np.arctan2(z,x)
            beta_deg= np.rad2deg(beta)
            theta_deg= np.rad2deg(theta)
            
            q21 = beta - psi;
            q21_deg= np.rad2deg(q21)
            q41 = theta - q31 - q21 
            q41_deg= np.rad2deg(q41)
            # Zweite Lösung:
            q32 = -q31;
            q22 = beta + psi; 
            q42 = theta - q31 + q21 
    ...
    Q = np.array([[q11, q21, q31, q41, qGripper, 0], [q12, q22, q32, q42, qGripper, 0]])
...
q = np.array([30, 40, 50, 10, 20, 30]) / 180 * np.pi + qC280
"""
...
"""
'''
{"A1":180.1757813,"T1":24,"A2":0.205078125,"T2":64,"A3":90.79101563,"T3":80,"A4":-0.439453125,"T4":48,"A5":179.6484375,"T5":-40}
'''
#RoArm
#qTest = np.array([180, 40, 90, -60 , 180, 0.0]) / 180 * np.pi + qRoArm
qTest = np.array([180, 0.0, 90, 0.0, 180, 0.0]) / 180 * np.pi + qRoArm


T_0_5 = fk_RoArm(dhRoArm, qTest)
print(T_0_5)
print(T_2_rotvec(T_0_5))


#[ 0.3470,  0.01396, 0.11809, 0.00002, 0.52351, -0.00009]


# inverse kinematik UR3

#q = np.array([0.27751, 0.01399, 0.27364, 0.0, 3.14159, 0.0])


#q = np.array([ 0.27832, 0.01357, 0.27658, 0.00005, 0.01946, -0.00533])
q = np.array([0.35333, 0.01399, 0.18969, 0.0, 2.79253, 0.0])



lösung = inv_RoArm(dhRoArm, q, np.pi)*(180/np.pi)
print("Das ist falsch:")
print(lösung[0] - qRoArm*(180/np.pi))
print(lösung[1] - qRoArm*(180/np.pi))
